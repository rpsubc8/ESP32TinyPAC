...
static uint8_t rb(void* userdata, uint16_t addr) 
{
  ...
  if (addr < 0x4000) 
  {
   ...
   //JJ return p->rom[addr];
    ...
    if (addr == 0x5003) {
      return gb_pac_flip_screen; //return p->flip_screen;
      ...
      return (!gb_pac_p1_up << 0) | (!gb_pac_p1_left << 1) | (!gb_pac_p1_right << 2) |
             (!gb_pac_p1_down << 3) | (!gb_pac_rack_advance << 4) | (!gb_pac_coin_s1 << 5) |
             (!gb_pac_coin_s2 << 6) | (!gb_pac_credits_btn << 7);
    } else if (addr >= 0x5040 && addr <= 0x507f) { // in 1
      return (!gb_pac_p1_up << 0) | (!gb_pac_p1_left << 1) | (!gb_pac_p1_right << 2) |
             (!gb_pac_p1_down << 3) | (!gb_pac_board_test << 4) | (!gb_pac_p1_start << 5) |
             (!gb_pac_p2_start << 6) | (1 << 7); // cabinet mode: 1=upright 0=table
...
static void wb(void* userdata, uint16_t addr, uint8_t val) 
{
  ...
  if (addr < 0x4000) {
    ...
    if (addr == 0x5000) {
      ...
      gb_pac_flip_screen = val & 1; //p->flip_screen = val & 1;
    } else if (addr == 0x5004 || addr == 0x5005) {
      // lamps, not used in pacman
    } else if (addr == 0x5006) {
      // coin lockout, not used in pacman
...
// if (x >= PAC_SCREEN_WIDTH) //Lo chequeo antes de llamar a draw_tile
...
static inline void draw_sprite(unsigned char sprite_no, uint8_t* pal, unsigned short int x, unsigned short int y, bool flip_x, bool flip_y) 
{
 ...
 //if (x > PAC_SCREEN_WIDTH) //No lo necesito, chequeo antes de llamar draw_sprite
 //{
 // return;
 //}
...
{  
  ...
  unsigned char *palette; //unsigned char palette[4];
  ...
  while (x != 31 || y != 36) 
  {
    ...
    xTile= ((x - 2) << 3);
    if (xTile < PAC_SCREEN_WIDTH)
    {
     draw_tile(tile_no, palette, xTile, (y<<3));//draw_tile(p, tile_no, palette, ((x - 2) << 3), (y<<3));
    }
  ...
  for (int s = 7; s >= 0; s--) {
    ...
    {
     draw_sprite(sprite_no, palette, x, y, flip_x, flip_y); //draw_sprite(p, sprite_no, palette, x, y, flip_x, flip_y);
    }
  }
...
//JJ  memset(p->ram, 0, sizeof(p->ram));
  ...
  memset(gb_pac_sprite_pos, 0, sizeof(gb_pac_sprite_pos)); //memset(p->sprite_pos, 0, sizeof(p->sprite_pos));
...
//  gb_ptr_id_rom[3]= gb_rom_pacman6j;
  ...
  {
   unsigned char data= gb_rom_82s1237f[i]; //p->color_rom[i];
