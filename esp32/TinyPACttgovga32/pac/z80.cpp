...
static inline void process_interrupts(z80* const z) {
  ...
  if (z->int_pending && z->iff1) {
    ...
    default:
      ...
      Serial.printf("unsupported interrupt mode %d\n", z->interrupt_mode);
      break;
    }
...
void exec_opcode(z80* const z, uint8_t opcode) {
  ...
  default: Serial.printf("unknown opcode %02X\n", opcode); break;
  }
}
...
void exec_opcode_dcb(z80* const z, uint8_t opcode, uint16_t addr) {
  ...
  default: Serial.printf("unknown XYCB opcode: %02X\n", opcode); break;
  }
...
void exec_opcode_ed(z80* const z, uint8_t opcode) {
  ...
  default: Serial.printf("unknown ED opcode: %02X\n", opcode); break;
  }
}
